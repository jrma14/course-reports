...
>
	...
	<p class="text-xs font-normal ml-5 mt-1 {helpVisible ? "visible" : "hidden"}">
		Current listings are just the courses that are available this year, so any cat II courses that do not occur this year will not be shown
	</p>
	<div class="flex mt-5">
		<div
			class="{showAllListings ? "" : selectedClass} w-32 flex justify-center items-center rounded-full mr-auto ml-auto"
		>
			Current Listings
		</div>
		<input
			type="checkbox"
			class="toggle"
			on:click={() => { showAllListings = !showAllListings;}}
			checked
		/>
		<div
			class="{showAllListings ? selectedClass : ""} w-32 flex justify-center items-center rounded-full mr-auto ml-auto"
